...
public class HorizontalSeekbarWithIntervals extends LinearLayout {
    ...
    public HorizontalSeekbarWithIntervals(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);

        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        inflater.inflate(R.layout.hor_seekbar_with_intervals, this);
    }
    ...
    private void alignFirstInterval(int offset) {
        ...
        int padding=  Math.round((getSeekbar().getPaddingLeft() - firstInterval.getWidth() / 2f));
        firstInterval.setPadding(padding, 0, 0, 0);
    }

    private void alignIntervalsInBetween(int maximumWidthOfEachInterval) {
        int seekbarwidth = getSeekbar().getWidth() - getSeekbar().getPaddingRight() - getSeekbar().getPaddingLeft();
        ...
        for (int index = 1; index < (getRelativeLayout().getChildCount() - 1); index++) {
            ...
            int width = textViewInterval.getWidth();
            float intheight = (seekbarwidth + width - width * (getRelativeLayout().getChildCount())) / (1f * (getRelativeLayout().getChildCount() - 1));
            ...
            textViewInterval.setPadding(padding, 0, 0, 0);
        }
    }
    ...
    private void alignLastInterval(int offset, int maximumWidthOfEachInterval) {
        ...
        int width = lastInterval.getWidth();

        int padding = Math.round(maximumWidthOfEachInterval - width  );
        lastInterval.setPadding(padding, 0, 0, 0);
    }
    ...
    private void alignFirstDevider() {
        ...
        int padding = Math.round(width);

        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) imgView.getLayoutParams();
        params.leftMargin = Math.round((getSeekbar().getPaddingLeft() - width / 2f));
    ...
    private void alignDeviders() {
        ...
        for (int index = 0; index < (getRelativeLayout_Deviders().getChildCount() - 1); index++) {
            ...
            int width = imgView.getWidth();
            float intheight = (seekbarWidth + width - width * (getRelativeLayout_Deviders().getChildCount())) / (1f * (getRelativeLayout_Deviders().getChildCount() - 1));
            int padding;
            if ((index & 1) == 0) {
                padding = (int) intheight;
            } else {
    ...
    private ImageView createDevider() {
        View boxView = (View) LayoutInflater.from(getContext())
                .inflate(R.layout.seekbar_deviders_h, null);
    ...
    private HorizontalSeekBar getSeekbar() {
        if (SeekBar == null) {
            SeekBar = (HorizontalSeekBar) findViewById(R.id.seekbar);
        }
